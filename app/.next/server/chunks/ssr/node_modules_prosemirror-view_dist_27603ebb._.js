...
"[project]/node_modules/prosemirror-view/dist/index.cjs [app-ssr] (ecmascript)": (function(__turbopack_context__) {
...
var webkit_version = webkit ? +(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent) || [
    0,
    0
])[1] : 0;
function windowRect(doc) {
...
function computeOuterDeco(outerDeco, node, needsWrap) {
    if (outerDeco.length == 0) return noDeco;
    var top = needsWrap ? noDeco[0] : new OuterDecoLevel(), result = [
        top
    ];
...
function selectionFromDOM(view) {
    ...
    if (selectionCollapsed(domSel)) {
        ...
        if (domSel instanceof view.dom.ownerDocument.defaultView.Selection && domSel.rangeCount > 1) {
            ...
            if (min < 0) return null;
            var _ref3 = max == view.state.selection.anchor ? [
                max,
                min
            ] : [
                min,
...
function stopNativeHorizontalDelete(view, dir) {
    ...
    var _view$state$selection2 = view.state.selection, $head = _view$state$selection2.$head, $anchor = _view$state$selection2.$anchor, empty = _view$state$selection2.empty;
    if (!$head.sameParent($anchor)) return true;
    if (!empty) return false;
    if (view.endOfTextblock(dir > 0 ? "forward" : "backward")) return true;
...
function defaultTripleClick(view, inside, event) {
    ...
    for(var i = $pos.depth + 1; i > 0; i--){
        ...
        if (node.inlineContent) updateSelection(view, prosemirrorState.TextSelection.create(doc, nodePos + 1, nodePos + 1 + node.content.size), "pointer");
        else if (prosemirrorState.NodeSelection.isSelectable(node)) updateSelection(view, prosemirrorState.NodeSelection.create(doc, nodePos), "pointer");
        else continue;
        return true;
    }
}
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
    ...
    if (from < 0) {
        ...
        if (newSel && !view.state.selection.eq(newSel)) {
            ...
            if (origin == "pointer") _tr.setMeta("pointer", true);
            else if (origin == "key") _tr.scrollIntoView();
...
//# sourceMappingURL=node_modules_prosemirror-view_dist_27603ebb._.js.map